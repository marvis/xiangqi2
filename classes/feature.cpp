...
int getFeature(const IplImage* image, const int k, FeatureMap **map)
{
	...
	(*map) = (FeatureMap *)malloc(sizeof(FeatureMap));
	(*map)->sizeX       = sizeX;
	(*map)->sizeY       = sizeY;
	(*map)->numFeatures = p;
	(*map)->map = (float *) malloc(sizeof (float) * 
		(sizeX * sizeY  * p));
	for(int i = 0; i < sizeX * sizeY * p; i++)
	{
		(*map)->map[i] = 0.0f;
	}
...
//normalization
    for(i = 1; i <= sizeY; i++)
    {
        for(j = 1; j <= sizeX; j++)
        {
			...
			if (valOfNorm==0.0)
			{
				for(ii = 0; ii < p; ii++)
				{
					newData[pos2 + ii        ] = alfa;
				}/*for(ii = 0; ii < p; ii++)*/
				//for(ii = 0; ii < 2 * p; ii++)
				//{
				//	newData[pos2 + ii + p * 4] = alfa;
				//}/*for(ii = 0; ii < 2 * p; ii++)*/
			}
			else
			{
            ...
            if (valOfNorm==0.0)
            {
				for(ii = 0; ii < p; ii++)
				{
					newData[pos2 + ii + p    ] = alfa;
				}/*for(ii = 0; ii < p; ii++)*/
				//for(ii = 0; ii < 2 * p; ii++)
				//{
				//	newData[pos2 + ii + p * 6] = alfa;
				//}/*for(ii = 0; ii < 2 * p; ii++)*/
            } 
            else
            {
   ...
   //         }/*for(ii = 0; ii < 2 * p; ii++)*/
			...
			if (valOfNorm==0.0)
			{
				for(ii = 0; ii < p; ii++)
				{
					newData[pos2 + ii + p * 2] = alfa;
				}/*for(ii = 0; ii < p; ii++)*/
				//for(ii = 0; ii < 2 * p; ii++)
				//{
				//	newData[pos2 + ii + p * 8] = alfa;
				//}/*for(ii = 0; ii < 2 * p; ii++)*/
			} 
			else
			{
            ...
            if (valOfNorm==0.0)
            {
				for(ii = 0; ii < p; ii++)
				{
					newData[pos2 + ii + p * 3 ] = alfa;
				}/*for(ii = 0; ii < p; ii++)*/
				//for(ii = 0; ii < 2 * p; ii++)
				//{
				//	newData[pos2 + ii + p * 10] = alfa;
				//}/*for(ii = 0; ii < 2 * p; ii++)*/
            } 
            else
            {
